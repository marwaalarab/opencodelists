...
def test_import_error_existing_release(
    coding_systems_database_tmp_dir, mock_bnf_import_data_path
):
    # set up an existing CodingSystemRelease and db file
    cs_release = CodingSystemRelease.objects.create(
    ...
    db_file = db_dir / "bnf_v_error_20221001.sqlite3"
    db_file.touch()
    assert db_file.exists()
    # raise an exception after the migrate command
    with patch(
        "coding_systems.bnf.import_data.csv.DictReader",
    ...
    assert not (db_dir / "bnf_v_error_20221001.sqlite3.bu").exists()
